...
def clip_dataset_list_groupby_time(grid_list, time):
    ...
    output_path = os.path.join(conf.sRslPath, conf.ID + "_" + time + '_4.tif')
    print(output_path)
    ...
    print("warped")
    tif_list = None
    mean = NdviCompute.ndvi_compute_byds(tif_dataset, os.path.join(conf.sRslPath, conf.ID + time + '_3' + conf.output_format), NdviCompute.IMAGE_TYPE_GF1)
    print("ndvi")
